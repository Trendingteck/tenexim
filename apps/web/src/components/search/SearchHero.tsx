...
export default function SearchHero({ defaultQuery, onSearch, initialFilters, compact }: SearchHeroProps) {
    ...
    return (
        <div className={`flex flex-col items-center justify-center p-8 bg-slate-50 dark:bg-slate-950 relative overflow-hidden transition-all duration-700 ${compact ? 'min-h-[220px] py-4' : 'min-h-[550px]'}`}>
            ...
            <div className={`w-full max-w-4xl relative animate-in fade-in slide-in-from-bottom-4 duration-700 z-10 ${compact ? 'scale-95 origin-center' : ''}`}>
                <div className={`text-center ${compact ? 'mb-4' : 'mb-10'}`}>
                    ...
                    <h1 className={`${compact ? 'text-2xl md:text-3xl mb-2' : 'text-5xl md:text-6xl mb-4'} font-black text-slate-900 dark:text-white font-serif tracking-tight leading-tight`}>
                        {compact ? 'Refined Search Grid ' : 'Decisions driven by '}
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-500 to-amber-600 italic">
                            {compact ? 'Filters' : 'verified reality.'}
                        </span>
                    </h1>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-2 relative z-20">
                    <div className="flex flex-col md:flex-row gap-2 border-b border-slate-100 dark:border-slate-800 pb-2 mb-2 p-2">
                        ...
                        <div className="relative flex-1 group">
                            <Globe className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 group-hover:text-amber-500 transition-colors" />
                            <select
                                value={country}
                    ...
                    <form onSubmit={handleSubmit} className="relative flex items-center p-2" ref={suggestionRef}>
                        ...
                        <input
                            ...
                            className={`flex-1 bg-transparent pl-14 pr-4 outline-none text-slate-900 dark:text-white font-bold placeholder:text-slate-400 ${compact ? 'py-1 text-sm' : 'py-4 text-xl'}`}
                        />
                        <Button className={`${compact ? 'h-10 px-5 text-[9px]' : 'h-14 px-8 text-[10px]'} rounded-xl font-black uppercase tracking-widest bg-slate-900 text-white hover:bg-slate-800 dark:bg-amber-500 dark:text-slate-900 dark:hover:bg-amber-400 shadow-lg`} onClick={() => handleSubmit()}>
                            Update Results
                        </Button>
