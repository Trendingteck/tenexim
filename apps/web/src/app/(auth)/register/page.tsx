...
import { registerAction, getGoogleAuthUrl } from '@/actions/auth';
...
export default function RegisterPage() {
    ...
    const handleGoogleSignup = async () => {
        ...
        try {
            ...
            console.error("Google Auth signup sequence handshaking error:", error);
            ...
            <div className="w-full lg:w-[45%] flex flex-col justify-between p-8 sm:p-12 lg:p-16 relative z-10 bg-white dark:bg-slate-950">
                ...
                <div className="w-full max-w-sm mx-auto my-auto py-10">
                    <div className="space-y-2 mb-8">
                        <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-850 rounded-full">
                            <Sparkles className="w-3 h-3 text-amber-600 dark:text-amber-400" />
                            <span className="text-[10px] font-bold text-amber-700 dark:text-amber-400 uppercase tracking-wider">30-Day Free Trial</span>
                        </div>
                        <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">
                            Create your account
                        </h1>
                        <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
                            Gain secure, instant access to over 2 billion trade records.
                        </p>
                    </div>
                    ...
                    <div className="relative flex py-5 items-center">
                        ...
                        <span className="flex-shrink mx-4 text-[10px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-widest">or register with email</span>
                        <div className="flex-grow border-t border-slate-100 dark:border-slate-800"></div>
                    </div>

                    {/* Server Action Form */}
                    <form action={formAction} className="space-y-4">
                        <input type="hidden" name="country_validation_field" />
                        ...
                        {state && state.success && (
                            <div className="p-3.5 bg-emerald-500/5 border border-emerald-500/10 rounded-xl flex items-start gap-2.5">
                                ...
                                <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 leading-normal">{state.message}</span>
                            </div>
                        )}
